...
basvuruRouter.patch('/:id/durum', asyncHandler(async (req, res) => {
  ...
  const [sonuc] = await pool.query(
    'UPDATE basvurular SET durum = :durum, karar_tarihi = NOW() WHERE id = :id',
    { id: basvuruId, durum },
  );
  if (!sonuc.affectedRows) throw ApiError.notFound('Başvuru bulunamadı');
